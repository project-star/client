...
var urlbuildThread = require('./urlbuild-thread');
 ...
 */
// @ngInject
function RootThread($rootScope, annotationUI, drafts, features, searchFilter, viewFilter, urlUI) {
   ...
   *        filter settings etc.)
   */
  function urlbuildRootThread(state) {
    var sortFn = sortFns[state.sortKey];

    var filterFn;
    if (state.filterQuery) {
      var filters = searchFilter.generateFacetedFilter(state.filterQuery);
      filterFn = function (url) {
        return viewFilter.filter([url], filters).length > 0;
      };
    }
    ...
    if (state.isSidebar && !state.filterQuery) {
      if (!features.flagEnabled('orphans_tab')) {
        ...
        threadFilterFn = function (thread) {
          ...
          if (metadata.isWaitingToAnchor(thread.url)) {
            return false;
          }
          switch (state.selectedTab) {
          case uiConstants.TAB_ANNOTATIONS:
            return metadata.isAnnotation(thread.url);
          case uiConstants.TAB_ORPHANS:
            return metadata.isOrphan(thread.url);
          case uiConstants.TAB_NOTES:
            return metadata.isPageNote(thread.url);
          default:
            throw new Error('Invalid selected tab');
          }
        };
    ...
    // Get the currently loaded annotations and the set of inputs which
    // determines what is visible and build the visible thread structure
    return urlbuildThread(state.urls, {
      forceVisible: truthyKeys(state.forceVisible),
      expanded: state.expanded,
      highlighted: state.highlighted,
      selected: truthyKeys(state.selectedUrlMap || {}),
      sortCompareFn: sortFn,
      filterFn: filterFn,
      threadFilterFn: threadFilterFn,
    });
  ...
  loadEvents.forEach(function (event) {
    $rootScope.$on(event, function (event, annotation) {
      ...
      if (event.name.match(/annotation.*/)){
          ...
          urlUI.addUrls([].concat(annotation));
       }
    });
  });
  ...
  this.thread = memoize(urlbuildRootThread);
}
