 ...
 */
function threadUrls(urlalls) {
  // Map of annotation ID -> container
  var threads = {};

  // Build mapping of annotation ID -> thread
  urlalls.forEach(function (url) {
    threads[id(url)] = Object.assign({}, DEFAULT_THREAD_STATE, {
      id: id(url),
      url: url,
      children: [],
    });
  ...
  urlalls.forEach(function (url) {
    ...
    setParentID(threads, id(url), url.references);
  });
...
function sort(threads, compareFn) {
  return threads.slice().sort(function (a, b) {
    ...
    if (compareFn(a.url, b.url)) {
      return -1;
    } else if (compareFn(b.url, a.url)) {
      return 1;
    } else {
      return 0;
    }
 ...
 */
function urlbuildThread(urlalls, opts) {
  opts = Object.assign({}, defaultOpts, opts);

  var thread = threadUrls(urlalls);
  ...
  // criteria
  var shouldShowThread = function (url) {
    if (opts.forceVisible && opts.forceVisible.indexOf(id(url)) !== -1) {
      return true;
    }
    if (opts.filterFn && !opts.filterFn(url)) {
      return false;
    }
    return true;
  };
