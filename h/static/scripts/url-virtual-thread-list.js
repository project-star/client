...
/**
 * URL VirtualThreadList is a helper for virtualizing the url thread list.
 *
 * 'Virtualizing' the thread list improves UI performance by only creating
 * url cards for urls which are either in or near the viewport.
 *
 * Reducing the number of annotation cards that are actually created optimizes
 * the initial population of the list, since url cards are big components
 ...
 *        threads.
 */
function UrlVirtualThreadList($scope, window_, rootThread) {
  var self = this;
...
inherits(UrlVirtualThreadList, EventEmitter);
 ...
 * is torn down.
 */
UrlVirtualThreadList.prototype.detach = function () {
  this._detach();
};
 ...
 */
UrlVirtualThreadList.prototype.setRootThread = function (thread) {
 ...
 * is used.
 *
 * @param {string} id - The url ID or $$tag
 * @param {number} height - The height of the url thread.
 */
UrlVirtualThreadList.prototype.setThreadHeight = function (id, height) {
...
UrlVirtualThreadList.prototype._height = function (id) {
...
UrlVirtualThreadList.prototype.yOffsetOf = function (id) {
  var self = this;
 ...
 */
UrlVirtualThreadList.prototype._updateVisibleThreads = function () {
  ...
  // Estimated height in pixels of url cards which are below the
...
module.exports = UrlVirtualThreadList;
